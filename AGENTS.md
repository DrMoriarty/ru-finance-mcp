...
- **Тесты есть (`pytest`), линтера/типчекера/CI нет.** Тесты лежат в `tests/`
  (`test_bonds`, `test_rate`, `test_fundamental`, `test_moex_utils`), конфиг —
  в `pyproject.toml` (`[tool.pytest.ini_options]`). Запуск:
  ```bash
  pip install -e '.[test]'
  pytest
  ```
  `ruff`/`mypy` не настроены — не предлагай без просьбы.
