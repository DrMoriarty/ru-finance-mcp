...
59 инструментов сервера разбиты на **роли**. В HTTP-режиме каждая роль доступна
по отдельному URL, чтобы агент получал только нужные инструменты (меньше контекста):

| Роль | URL | Инструментов | Описание |
|------|-----|-------------|----------|
| **all** | `/mcp` | 59 | Все инструменты (как раньше) |
| **market** | `/market/mcp` | 24 | Котировки, история, волатильность, ликвидность, дивиденды |
...
Stdio-режим отдаёт все 59 инструментов (роли не разделены).
