...
- **Модели в `ru_finance/`:** `session.py` (клиент aioboy/moex + ретраи на сырой requests/niquests),
  `moex.py` (резолв/котировки/облигации/история + CCI/НРД-эндпоинты: компании,
  ...
  smart-lab.ru). Наружу всё пробрасывается как
  инструменты в `mcp_server.py` (36 `@mcp.tool()`).
