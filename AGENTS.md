...
66 инструментов сервера разбиты на **роли**. В HTTP-режиме каждая роль доступна
по отдельному URL, чтобы агент получал только нужные инструменты (меньше контекста):

| Роль | URL | Инструментов | Описание |
|------|-----|-------------|----------|
| **all** | `/mcp` | 67 | Все инструменты (как раньше) |
| **market** | `/market/mcp` | 28 | Котировки, история, волатильность, ликвидность, дивиденды, ETF/БПИФ |
| **bond** | `/bond/mcp` | 20 | Облигации, скринер, кредитные рейтинги, ЗПИФ |
...
Stdio-режим отдаёт все 67 инструментов (роли не разделены).
